...
cap = cv.VideoCapture("C:/Users/sinji/Desktop/firstick/src/wood01.mp4")
cnt = 1
...
while True :    
    ...
    blur_ = cv.GaussianBlur(gray, (5, 5), 0)
    canny = cv.Canny(blur, 800, 2000, apertureSize = 5, L2gradient = True)
    canny_ = cv.Canny(blur_, 800, 2000, apertureSize = 5, L2gradient = True)
    ...
    if lines is not None :
        for i in lines :
            (a21, b21, a22, b22) = i[0]
            if abs((b22 - b21)/(a22 - a21)) < 5 :  # 각도 왜 안 돼??
                # cv.line(resize, (a21, b21), (a22, b22), (0, 255, 255), 2)
                part = a22 - a21

    # 점 찍기
    if lines is not None :
        if whole * 0.6 < part < whole * 0.7 :
            print("True", cnt)
            cnt += 1
